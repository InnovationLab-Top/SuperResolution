...
def load_img(filepath, nFrames, scale, other_dataset):
    ...
    if other_dataset:
        ...
        input=target.resize((int(target.size[0]/1),int(target.size[1]/1)), Image.BICUBIC)
        ...
        for i in seq:
            ...
            if os.path.exists(file_name):
                temp = modcrop(Image.open(filepath[0:char_len-7]+'{0:03d}'.format(index)+'.png').convert('RGB'),scale).resize((int(target.size[0]/1),int(target.size[1]/1)), Image.BICUBIC)
                neigbor.append(temp)
            else:
        ...
        input = target.resize((int(target.size[0]/1),int(target.size[1]/1)), Image.BICUBIC)
        neigbor = [modcrop(Image.open(filepath+'/im'+str(j)+'.png').convert('RGB'), scale).resize((int(target.size[0]/1),int(target.size[1]/1)), Image.BICUBIC) for j in reversed(seq)]
    ...
    if other_dataset:
        ...
        for i in seq:
            ...
            if os.path.exists(file_name1):
                temp = modcrop(Image.open(file_name1).convert('RGB'), scale).resize((int(target.size[0]/1),int(target.size[1]/1)), Image.BICUBIC)
                neigbor.append(temp)
            else:
                print('neigbor frame- is not exist')
                temp=input
        ...
        #random.shuffle(seq) #if random sequence
        for j in seq:
            neigbor.append(modcrop(Image.open(filepath+'/im'+str(j)+'.png').convert('RGB'), scale).resize((int(target.size[0]/1),int(target.size[1]/1)), Image.BICUBIC))
    return target, input, neigbor
